...
"""
Analizador Léxico
Curso: Compiladores y Lenguajes de Bajo de Nivel, 1ndo Sem, 2017
...
def siguiente_token(f, num_linea=1):
    ...
    if c in [' ', '\t']:
        ...
        while c.isalpha():
            ...
            if token.lexema.lower() not in ['true', 'false', 'null']:
                err_msg = u'Token inválido en linea %s: "%s"' % (num_linea, token.lexema)
                raise CaracterInvalido(err_msg)
        ...
        if estado in estados_de_aceptacion and c in caracteres_esperados:
            ...
            err_msg = u'Caracter inesperado en linea %s: "%s"' % (num_linea, c)
            CaracterInvalido(err_msg)

    if token is not None and token.complex is None:
        err_msg = u'Caracter inesperado en linea %s: "%s"' % (num_linea, c)
        raise CaracterInvalido(err_msg)
